...
DEP_LFQ_QC <- function(input_matrix,dataset_name){
    ...
    input_matrix <- Metabolomics   %>%
        dplyr::select(!matches("noco")) 
    ...
    data_unique_Etop <- input_matrix %>% as.data.frame() %>%  rownames_to_column("name") %>% 
 ...
 # ggsave(here::here("Output","Figures","Metabolites_Missingness_qc.pdf"))   
    ...
    plot_imputation(data_norm, data_norm)
    ...
    data_diff_all_contrasts <- DEP::test_diff(data_imp, type = "manual", 
                                              test = c("prdx1_ko_etop_0_vs_prdx1_ko_ut", 
                                                       ...
                                                       "prdx1_ko_etop_24h_vs_prdx1_ko_etop_0",
                                                     "prdx1_ko_etop_24h_vs_prdx1_ko_ut",
                                                       "prdx1_ko_etop_8h_vs_prdx1_ko_etop_0"  ,
                                                        "prdx1_ko_etop_24h_vs_prdx1_ko_etop_8h"))
    # data_diff_all_contrasts <- DEP::test_diff(data_imp, type = "manual", 
    #                                           test = c("prdx1_ko_etop_0_vs_prdx1_ko_ut", 
    #                                                    "prdx1_ko_etop_8h_vs_prdx1_ko_etop_0",
    #                                                    ))
    ...
    #     rowwise() %>% 
    #     mutate(prdx1_ko_ut= base::mean(c(prdx1_ko_ut_1, prdx1_ko_ut_2, prdx1_ko_ut_3), na.rm = T)) %>% 
    ...
    # 
    # dplyr::mutate(dplyr::across(everything(),~.x- prdx1_ko_ut)) %>%
    #     dplyr::select(-prdx1_ko_ut) %>%
    #     dplyr::select(!matches("wt|noco")) %>%
    #     as.matrix()
      Significant_proteins <- data_imp@assays@data@listData[[1]] %>%
            as.data.frame() %>%
            rownames_to_column("Metabolite") %>%
            ...
            pivot_wider(names_from = "Condition",values_from = "Mean_Abundance") %>%
            dplyr::mutate(duplicated = BiocGenerics::duplicated(Metabolite)) %>%
        ungroup %>%
            column_to_rownames("Metabolite") %>%
            dplyr::select(where(is.numeric)) %>%
            dplyr::mutate(across(where(is.numeric),~.x- prdx1_ko_ut)) %>%
            dplyr::select(-prdx1_ko_ut) %>%
            as.matrix()
    paletteLength <- 50
    ...
    annotation_row= data.frame(name = rownames(Significant_proteins)) %>% 
        ...
        .[c(3,5,1)]%>% rownames_to_column("Metabolite")
    pathway <- data.table(
      Metabolite =  annotation_row$Metabolite,
      Pathway = c("Nucleotide metabolism","Arginine biosynthesis","Nucleotide metabolism","Glycolysis",
                  "Pentose phosphate pathway","Nucleotide metabolism","Amino sugar and nucleotide sugar metabolism","Glycolysis",
                  "Nucleotide metabolism","Pyruvate metabolism","Nucleotide metabolism","Glycerophospholipid metabolism",
                  "beta-Alanine metabolism","Citrate cycle (TCA cycle)","Nucleotide metabolism","Nucleotide metabolism"))
    # annotation_row <- annotation_row[,c(3,1,2)]
    ...
    pdf(here::here(output_folder,glue::glue("Heatmap_Significant ","PRDX1",".pdf")),width = 10,height = 10) 
    # Significant_proteins <- as.data.frame(lapply(Significant_proteins,
    #                                           function(x) as.numeric(x)))
    # annotation_row<- annotation_row %>% dplyr::mutate(across(everything(),as.numeric))
    Significant_Etop <- pheatmap::pheatmap(Significant_proteins[rownames(annotation_row),c(1,3,2)],
                                           cluster_cols = F,fontsize_row = 10, cluster_rows = F,
...
DF_anova <- Savvas_anona_omu(count_data =  Anova_test2, 
 ...
 interesting_metabolites <- c("adenosine_monophosphate","uridine_monophosphate","inosine_triphosphate","guanosine_triphosphate", "aspartic_acid","gamma_glu_cys")
